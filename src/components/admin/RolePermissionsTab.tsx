                  ...
                  {resources.map((resource) => (
                    <TableRow key={resource}>
                      ...
                      {actions.map(action => {
                        ...
                        const isAssigned = targetPerm ? (activeRole.permissions || []).some(p => p.permission_id === targetPerm.permission_id) : false;
                ...
                <div className="rounded-lg border divide-y">
                  {(!detailRole.permissions || detailRole.permissions.length === 0) ? (
                    ...
                    detailRole.permissions.map((perm, idx) => {
                       const fullPerm = allPermissions.find(ap => ap.permission_id === perm.permission_id) || perm;
                       return (
